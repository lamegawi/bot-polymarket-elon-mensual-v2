...
cron) hace:
  ...
  2) Si no hay apuesta activa: evalúa las señales en vivo (mismas reglas
     R1-R7) y, si hay candidata (solo mercados mensuales), la ABRE con el
...
ESTADO = "papel_mensual.json"
HISTORIAL = "resultados_papel_mensual.csv"
BANKROLL = 500.0
...
def resolver(estado):
    ...
    try:
        notificar.apuesta_cerrada(registro, estado["saldo"], prefijo="[V2-MENSUAL] ")
    except Exception:
        pass
    return True
...
def abrir(estado, actualizar=False):
    """Evalúa señales y abre apuesta de papel si hay candidata mensual."""
    try:
    ...
    if not candidatas:
        print(f"  (sin señal mensual → no se abre apuesta de papel · paso actual: {estado['paso']})")
        return False
    ...
    try:
        notificar.apuesta_abierta(estado["activa"], estado["saldo"], prefijo="[V2-MENSUAL] ")
    except Exception:
        pass
    return True
...
def pasada(actualizar=False, excel=False):
    ...
    print(f"[{datetime.now(ET).strftime('%Y-%m-%d %H:%M %Z')}] Paper trading MENSUAL · "
