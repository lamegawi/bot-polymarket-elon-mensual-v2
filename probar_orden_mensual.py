...
mercado mensual activo. Es un precio IMPOSIBLE (los bins cotizan muy
...
from operar_real_mensual import cargar_config, get_client, token_id_para_bin
...
def main():
    ...
    print("Buscando mercado mensual activo…")
    try:
        ...
        return 1
    activo = next((m for m in mercados if not m["cerrado"] and m["tipo"] == "mensual"), None)
    if not activo:
        print("  ✖ no hay mercado mensual abierto ahora mismo (reintenta más tarde)")
        return 1
